...
#### 4.1.1 shadcn/ui 도입 (2026-09-06)

UI 기반을 shadcn/ui로 옮겼다. **다만 `npx shadcn init`은 실행하지 않는다** — 샌드박스에서 실제로 돌려 컴파일된 CSS를 대조한 결과, `app/globals.css`와 `app/layout.tsx`를 덮어쓰며 (a) `--radius-*` 스케일을 주입해 `rounded-*` 261곳이 ±2px 이동하고 (b) `layout.tsx`에 Geist를 넣어 `--font-sans`를 덮어써 한글 Noto Sans KR을 잃는다. 이를 막는 플래그는 없다.

대신 다음 구조를 손으로 구성한다.

- `components.json`(shadcn 설정) + `lib/utils.ts`의 `cn()`. 이후 `npx shadcn add`는 정상 동작한다.
- `app/globals.css` 끝에 **추가만 하는** 시맨틱 토큰 브리지. 기존 토큰은 하나도 바꾸지 않고, `--radius-*` 스케일과 `.dark` 블록은 넣지 않는다.
- `components/ui/*`는 shadcn 형태(`data-slot`·`cva`·`cn`·ref as prop)를 따르되 **base 문자열은 shadcn 기본값을 쓰지 않는다.** 기본 base는 이 디자인을 깬다: `disabled` 시 포인터 이벤트를 죽여 19곳의 커서 표시를 무력화하고, 줄바꿈 금지로 드롭다운·온보딩 카드 6곳이 넘치며, 이 디자인에 없는 포커스 링을 추가하고, 기본 모서리·글자 크기·굵기가 93개 버튼 대부분과 충돌한다. variant 테이블은 **이 코드베이스의 실제 클래스 문자열에서 역추출**한다.
- 회귀 판정 기준은 **컴파일된 스타일시트의 동일성**이다. 마이그레이션은 클래스 리터럴을 화면에서 테이블로 옮길 뿐이므로, 앱 전체가 쓰는 클래스 집합은 변하지 않아야 한다. `npx @tailwindcss/cli -i app/globals.css -o <out>` 결과가 바이트 단위로 같아야 한다.

**승인된 시각 변경 1건** — 온보딩 선택 카드 4곳(`OnboardingScreen.tsx`의 사업자 유형·채용 예정·면세사업자·step8 항목). 원본은 `CARD`의 `border-[#E4E6EA]`와 `CARD_ON`의 `border-[#6E62C2]`를 동시에 붙이는데, 컴파일된 스타일시트에서 회색 규칙이 뒤에 와서 **선택 상태인데도 회색 테두리**로 그려졌다. `cn()`을 거치면 나중 인자인 브랜드 보라가 이겨 `CARD_ON`이 의도한 대로 렌더링된다. 사용자 승인을 받아 이 동작을 채택했다(2026-09-06).
...
- **추가는 허용**한다 — 오버레이·토스트·포커스 상태·애니메이션처럼 원본에 아예 없던 것.
- **기존 화면을 조용히 다시 칠하는 것은 여전히 금지**한다. 이미 완성된 카드·여백·팔레트는 건드리지 않는다.
- 회귀 판정 기준을 바꾼다: 컴파일된 스타일시트의 **동일성**이 아니라 **기존 선언의 불변성**이다. 규칙 추가는 정상이고, 기존 선언이 제거·변경되면 실패다.
  ```
  diff -u <원본> <현재> | grep -E '^-[^-]'   # 아무것도 출력되지 않아야 한다
  ```
...
| 컴포넌트 | 사용처 |
| --- | --- |
| AlertDialog | 마이페이지 프로필 초기화 · 할 일 삭제 |
| DropdownMenu | 온보딩 데모 프로필 선택 |
| Tooltip | 온보딩 업종·상시근로자 수·면세사업자 |
| Select | MiniForm · TaskForm · 현금흐름 시트/열 역할 |
| Switch · Checkbox | 마이페이지 알림 토글 4개 · 할 일 완료 |
| Toaster(sonner) | AI 초안·중복판정·파싱·인터뷰·조건코치 등 7개 파일 |

다크 모드는 여전히 지원하지 않는다. `next-themes`는 설치하지 않는다(`shadcn add sonner`가 넣으므로 제거해야 한다).
...
## §17 회원 정보 수집 — 설문형 (2026-09-19 추가)

§15의 대화형 온보딩(AI 인터뷰)을 없애고 설문으로 바꿨다. 회원 정보 수집에는 LLM을 쓰지 않는다.

1. **흐름** — `/signup` → `/onboarding`(설문). `/onboarding/chat`은 `/onboarding`으로 리다이렉트만 한다. 프로필이 없는 로그인 사용자도 `/onboarding`으로 보낸다. `POST /api/ai/interview`, `InterviewScreen`, `interviewCoerce`, 인터뷰 프롬프트·스키마는 삭제했다.
2. **설문** — 16문항, 섹션 3개(기본 정보 · 대표자·자격 · 사업 현황). 필수는 사업자 형태·업종·지역·개업일·상시근로자 수 5개. 한 화면에 한 질문이며, 단일 선택은 고르면 다음 질문으로 넘어간다. 선택 항목은 "모름"·"답하지 않음"·건너뛰기가 가능하고, 모두 §0.1-7에 따라 확인 필요로 판정된다.
3. **판정 미리보기** — 필수 5개가 채워지면 옆 패널에서 대상·확인 필요·조건 하나 부족·제외 건수를 실시간으로 보여준다(마감 공고 제외 — 판정함과 같은 기준). 확인 필요 요건을 확정하려면 답해야 할 항목을 걸린 공고 수가 많은 순으로 보여주고, 누르면 그 질문으로 이동한다.
4. **질문의 근거** — 각 질문 아래에 "공고 N건 중 M건의 자격 판정에 쓰입니다"를 카탈로그에서 계산해 보여준다.
5. **이어쓰기** — 작성 중인 답은 `bizbuddy:survey:v1`(기기 로컬, 서버 동기화 제외)에 저장하고, 저장을 마치면 비운다.
6. **수정** — `/onboarding?edit=1`은 저장된 프로필로 같은 설문을 확인 화면부터 연다. 항목을 누르면 그 질문으로 갔다가 확인 화면으로 돌아온다.
7. §0.1-4의 인터뷰 예외(회사 정보가 AI 응답 생성을 위해 서버를 경유)는 더 이상 없다.

## §18 신청서 초안 고도화 (2026-09-19 추가)

§15의 신청서 뼈대를 "자동으로 최대한 채우는 초안"으로 확장했다. LLM은 여전히 공고문만 본다.

1. **내 사업 정보** (`lib/draft/library.ts`, `bizbuddy:library:v1`, 계정 동기화 키 `library` 추가) — 문제·고객·제품·차별성·시장·성과·팀·일정·자금·기대효과 10개 서술형 답. 템플릿의 `{{lib:키}}`로 참조하며, 한 번 쓰면 모든 공고의 초안에 들어간다. LLM에는 보내지 않는다.
2. **자격 충족 근거** (`lib/draft/evidence.ts`) — 판정 엔진의 요건별 결과로 "공고 기준 '…' — 현재 … (충족)" 문단을 만든다(`{{eligibility_summary}}`). 확인 필요 요건은 빈칸, 미충족 요건은 "놓치기 쉬운 것" 경고로 뺀다.
3. **기본 양식** (`lib/draft/template.ts`) — AI 없이 분야별 표준 목차로 즉시 초안을 만든다. 창업은 PSST(문제 인식·실현 가능성·성장 전략·팀 구성), R&D·고용·수출·일반은 각자의 목차. 공고 지정 서식이 있으면 그 서식이 우선이라는 경고를 붙인다. 서버에 Gemini 키가 없으면 AI 생성 대신 기본 양식을 안내한다.
4. **빈칸 채우기 패널** — 이 초안이 쓰는 내 사업 정보 항목과 이 공고에만 쓰는 [[빈칸]]을 모아 바로 입력받는다. 사용자가 이미 고친 문단의 `[[입력 필요: 라벨]]`도 나중에 생긴 값으로 다시 채운다(`refill`).
5. **평가항목 점검** — 문단마다 다루는 평가항목(`sections[].criteria`, AI 스키마에도 추가)을 두고, 항목별로 관련 문단과 남은 빈칸을 보여준다. 전체 채움 비율과 문단별 글자 수를 표시한다.
6. **내보내기** — 기존 Markdown에 더해 서식 없는 본문 복사(한글·워드 양식 붙여넣기용).
