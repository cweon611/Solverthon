...
3. 각 section.template은 실제 신청서에 들어갈 문단의 뼈대입니다. 평가항목과 연결된 문단은 채웠을 때 600~800자가 나오도록 도입 문장과 빈칸을 충분히 둡니다.
   추진 일정·자금 계획·기대효과를 다루는 문단은 제목에 각각 "일정", "자금", "기대효과"를 넣습니다. 서비스가 그 문단에 표(추진 일정표·자금 소요표·정량 목표표)를 자동으로 붙입니다. 표는 직접 만들지 않습니다.
   ...
   - 사업 서술 중 아래 항목에 해당하는 자리는 {{lib:키}}로 씁니다. 사용자가 한 번 써 둔 문장이 여러 신청서에 재사용됩니다.
     ...
     {{lib:키}}에는 사용자가 쓴 완결된 문장·문단이 통째로 들어갑니다. 그래서 {{lib:키}}는 문장 중간에 넣지 말고 반드시 줄을 바꿔 단독으로 둡니다. 앞에 짧은 소제목을 붙이는 것은 됩니다.
     {{lib:키}} 바로 앞 줄에는 그 내용을 소개하는 한 문장을 둡니다. 꼬리표("목표 고객")가 아니라 문장이어야 합니다.
     좋은 예: "본 사업의 목표 고객은 다음과 같습니다.
{{lib:customer}}"   나쁜 예: "당사는 {{lib:customer}}을 주요 타깃으로 설정하고"
   - 첫 문단(기업 개요)에는 {{company_intro}}를 씁니다. 회사 이름·지역·업종·업력·인원과 충족한 자격 요건이 한 문장으로 들어갑니다.
